...
import { adminService, PlatformStats, SubscriptionOverview, AdminUser } from '@/services/adminService';
...
export const AdminDashboard: React.FC = () => {
  ...
  const churnRate =
    subs && subs.total_active + subs.total_canceled > 0
      ? ((subs.total_canceled / (subs.total_active + subs.total_canceled)) * 100).toFixed(1)
      : '0.0';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary">Vista general de la plataforma</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            />
            <p className="text-xs text-text-secondary uppercase tracking-wide">Usuarios Pagados</p>
          </div>
          <p className="text-3xl font-black text-text">{loading ? '...' : paidUsers}</p>
          <div className="mt-2">
            <span
              className={clsx(
        ...
        <div className="bg-card rounded-3xl border border-border p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-text-secondary uppercase tracking-wide">Total Eventos</p>
          </div>
          <p className="text-3xl font-black text-text">
        ...
        <div className="bg-card rounded-3xl border border-border p-5 shadow-sm">
          ...
          <div className="flex items-end gap-1.5">
            ...
            <span className="text-xl text-text-secondary font-bold mb-0.5">/</span>
            <p className="text-3xl font-black text-text">
              {loading ? '...' : stats?.total_products ?? 0}
            </p>
          </div>
          <p className="text-xs text-text-secondary mt-2">en toda la plataforma</p>
        </div>
      </div>
        ...
        >
          <div
            className={clsx(
              ...
              !loading && parseFloat(churnRate) > 20 ? 'bg-error/10' : 'bg-surface-alt',
            )}
          >
            <XCircle
              className={clsx(
                'h-5 w-5',
                !loading && parseFloat(churnRate) > 20 ? 'text-error' : 'text-text-secondary',
              )}
            />
          </div>
          <div>
            ...
            <p
              className={clsx(
                ...
                !loading && parseFloat(churnRate) > 20 ? 'text-error' : 'text-text',
              )}
            >
              {loading ? '...' : `${churnRate}%`}
            </p>
            <p className="text-xs text-text-secondary">subs canceladas vs total</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-card shadow-sm border border-border rounded-3xl p-6">
          <h3 className="text-base font-semibold text-text mb-4">Distribución de Planes</h3>
          <div className="h-56">
            {loading ? (
        ...
        {/* Signups Bar Chart */}
        <div className="bg-card shadow-sm border border-border rounded-3xl p-6">
          <h3 className="text-base font-semibold text-text mb-4">Nuevos Registros</h3>
          <div className="h-56">
            {loading ? (
              <div className="h-full flex items-center justify-center">
                <RefreshCw className="h-8 w-8 animate-spin text-gray-300" />
      ...
      <div className="bg-card shadow-sm border border-border rounded-3xl p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-text">Top Usuarios por Actividad</h3>
          </div>
          <Link
            to="/admin/users"
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-14 rounded-xl bg-surface-alt animate-pulse" />
            ))}
          </div>
        ) : topUsers.length === 0 ? (
          <div className="text-center py-10">
            <Users className="h-9 w-9 text-text-secondary opacity-25 mx-auto mb-3" />
            <p className="text-sm text-text-secondary">No hay usuarios registrados aún.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {topUsers.map((user, idx) => {
              const score = user.events_count + user.clients_count + user.products_count;
              const maxScore =
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-3 text-xs text-text-secondary shrink-0">
                        ...
                        <span className="font-bold text-text" title="Actividad total">
                          {score} total
                        </span>
                      </div>
                    </div>
            ...
            {stats.new_users_week >= stats.new_users_month / 4 ? (
              <>
                ...
                <p className="text-sm text-text-secondary">
                  ...
                  {Math.round(stats.new_users_month / 4)} promedio semanal del mes.
                </p>
              </>
            ) : (
              <>
