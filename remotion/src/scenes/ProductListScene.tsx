...
const PRODUCTS = [
  { initials: 'PT', name: 'Pastel 3 Pisos', category: 'Pastelería', price: '$2,500.00' },
  { initials: 'DC', name: 'Decoración Completa', category: 'Decoración', price: '$8,000.00' },
  { initials: 'BQ', name: 'Banquete 100 pax', category: 'Banquete', price: '$15,000.00' },
];

const PackageIcon: React.FC<{ size?: number }> = ({ size = 16 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M16.5 9.4L7.5 4.21M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z" stroke={COLORS.primary} strokeWidth="1.5" />
    <path d="M3.27 6.96L12 12.01l8.73-5.05M12 22.08V12" stroke={COLORS.primary} strokeWidth="1.5" />
  </svg>
);
            ...
            <div style={{ flex: 1, padding: '0 40px 40px', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 12, border: `1px solid ${COLORS.border}`, backgroundColor: COLORS.card, color: COLORS.text, fontSize: 14, fontWeight: 500 }}>
                    <DownloadIcon />
                    CSV
                  </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
                {['Todas', 'Pastelería', 'Decoración', 'Banquete'].map((cat, i) => (
                  <div key={cat} style={{
                    padding: '4px 14px',
                    borderRadius: 9999,
                    fontSize: 12,
              ...
              <div style={{ backgroundColor: COLORS.card, overflow: 'hidden', borderRadius: 24, border: `1px solid ${COLORS.border}` }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1.5fr', backgroundColor: COLORS.surfaceAlt }}>
                  {['PRODUCTO', 'PRECIO', 'CATEGORÍA'].map((col) => (
                ...
                {PRODUCTS.map((product, i) => {
                  ...
                  return (
                    <div key={product.name} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1.5fr', borderTop: i > 0 ? `1px solid ${COLORS.border}` : undefined, transform: `translateX(${translateX}px)`, opacity: rowOpacity }}>
                      <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: `${COLORS.primary}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <PackageIcon />
                        </div>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{product.name}</div>
                          <div style={{ fontSize: 12, color: COLORS.textSecondary, marginTop: 2 }}>{product.category}</div>
                        </div>
                      </div>
                      <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center' }}>
                        <span style={{ fontSize: 14, fontWeight: 600, color: COLORS.text }}>{product.price}</span>
                      </div>
                      <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center' }}>
                        <span style={{ padding: '2px 10px', borderRadius: 9999, backgroundColor: COLORS.surfaceAlt, color: COLORS.textSecondary, fontSize: 12 }}>{product.category}</span>
                      </div>
                    </div>
                  );
                })}
