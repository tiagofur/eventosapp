...
const getEventDateText = (eventDate: string): string => {
  try {
    return format(parseISO(eventDate), "d 'de' MMMM 'de' yyyy", { locale: es });
  } catch {
    return eventDate;
  }
};
...
export const renderContractTemplate = ({
  ...
  if (strict && missingTokens.size > 0) {
    throw new ContractTemplateError(
      "Faltan datos para completar la plantilla del contrato.",
      [],
      Array.from(missingTokens),
    );
  }
