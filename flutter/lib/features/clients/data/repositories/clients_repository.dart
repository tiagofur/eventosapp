...
class ClientsRepository {
  ...
  Future<List<ClientEntity>> getClients({String? search}) async {
    try {
      ...
      return clientsList
          .map((json) => ClientModel.fromJson(json as Map<String, dynamic>).toEntity())
          .toList();
    } catch (e) {
      rethrow;
    }
  ...
  Future<ClientEntity> updateClient(String id, Map<String, dynamic> data) async {
    try {
  ...
  Future<List<ClientPayment>> getClientPayments(String clientId) async {
    ...
    return paymentsList.map((json) => ClientPayment(
